...
[[https://diataxis.fr/][Diátaxis]] style (do not use this page as the primary guide):

- *Tutorial* (learning, end-to-end XYZ→CON→selection): [[file:chemfiles-tutorial.org][chemfiles-tutorial.org]]
- *How-to* (task recipes, batch convert, C API): [[file:chemfiles-howto.org][chemfiles-howto.org]]
- *Explanation* (why optional chemfiles, bonds in JSON, index remap): [[file:chemfiles-explain.org][chemfiles-explain.org]]
- *Reference* (API tables, grammar subset, install matrix): [[file:chemfiles-reference.org][chemfiles-reference.org]]
