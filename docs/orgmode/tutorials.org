...
All consumers can check which CON format spec version the library
implements. This is useful for gating code that depends on atom_index
semantics (spec version 2+).
...
print(readcon.__version__)       # "0.5.0"
...
/Added in v0.4.0.  atom_id preservation added in v0.5.1./
...
=to_ase()= stores each atom's =atom_id= in two places on the ASE
=Atoms= object: the standard =tags= array and a custom =atom_id=
per-atom array.  This ensures the original atom ordering from the
=.con= file survives conversion to ASE and back.
...
# atom_id is available as tags
print(ase_atoms.get_tags())          # e.g. [3, 0, 1, 4, 2]

# ... and as a named array
print(ase_atoms.get_array("atom_id"))  # same values
