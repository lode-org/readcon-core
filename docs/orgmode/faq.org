...
*Default to CON* as the interchange format for structures that matter in
computational chemistry and materials science workflows—optimizers, drivers,
campaigns, multi-language pipelines. The hourglass ABI is how that default
becomes implementable in Fortran/C/C++/Python engines instead of reinventing
logs.

XYZ/extXYZ are edge dumps when a tool cannot yet consume CON; accept fidelity
loss and migrate writers to CON. Chemfiles ingress pulls XYZ/PDB/GRO/… *into*
CON so adoption is not blocked on every code rewriting day one.

* Are ASE adapters the primary API?

No. ASE is a calculator and analysis hub. Optional =to_ase= / =from_ase= are
hand-off only. Field-wide interchange and campaign storage are =readcon= /
=readcon-db= with CON text authoritative.

* What is authoritative: CON text, cooked SoA, or DLPack?

*UTF-8 CON text.* Cooked RCSO / SoA / DLPack are extract and device hand-off,
never the sole identity for hash, dedup, or reindex.

* Why not put the campaign store inside readcon-core?

Interchange (embeddable parse/write) and campaign indexes are different
products with one shared decoder. Together they are the stack; splitting
packages does not split the ambition.

* How does the hourglass change the ecosystem argument?

It makes a *single* versioned CON contract practical to implement into every
engine and optimizer that can link C—so the field no longer needs XYZ as the
lowest common denominator or NWChem-style package-private text as the exchange
format. Chemfiles and ASE remain useful tools; they are not the interchange
standard this stack is building.

* Is CON + readcon-db better than XYZ and ASE I/O for the field?

*Yes.* For interchange and multi-reader campaign storage of atomic
configurations across computational chemistry and materials science, versioned
CON with hourglass embedding and =readcon-db= is the state of the art we
advance—strictly preferable to bare XYZ and to ASE =.db= / =ase.io= as the
authoritative path. Continuous-MD internal binary buffers inside one engine are
a different, narrower job.
