...
builds). When enabled, readcon-core can import chemfiles trajectories into
=ConFrame= (topology becomes =metadata["bonds"]= when present, with bond
indices remapped into type-grouped =atom_data= order via =atom_id=) and
evaluate chemfiles selection strings on frames via Rust, C/C++, Python, and
Julia (=select_on_frame= / =rkr_frame_select=).
...
(see =chemfiles_selection_cpp_regression=). Gaps: single CON symbol label
(not separate chemfiles name/type), residues/properties/numeric geometry
blocks from full chemfiles =selection.cpp= unless trivially projected.
