...
Default to *CON* for optimizer checkpoints and multi-language interchange:
constraints, forces/velocities sections, versioned metadata, and round-trips
through Fortran/C/C++/Python via the hourglass ABI.

Use *XYZ / extXYZ* only as an edge dump when a tool cannot consume CON—and
accept the fidelity loss. Prefer writing CON from optimizers/engines over
treating XYZ as durable identity.

Use *chemfiles ingress* to pull XYZ/PDB/GRO/… *into* =ConFrame= / CON when
foreign inputs must join a CON-centered pipeline.

Timings: [[file:benchmarks.org][benchmarks.org]].

* Are ASE adapters the primary API?

No. Optional =to_ase= / =from_ase= are *calculator hand-off*. Campaign storage,
hash/dedup, and multi-reader screening use =readcon= / =readcon-db= with CON
text authoritative. ASE is excellent for calculators; it is not the hourglass
interchange layer for CON.

* What is authoritative: CON text, cooked SoA, or DLPack?

*UTF-8 CON text* is durable authority for files and =readcon-db= =frames=.
Optional cooked RCSO / =frames_soa= and in-process SoA / DLPack accelerate
numerics and device hand-off; they are discardable or ephemeral. Default builds
reject non-CPU DLPack device requests unless optional =cuda= is enabled.

* Why not put the campaign store inside readcon-core?

Interchange (parse/write/embed) and campaign indexes are different jobs.
=readcon-core= embeds via the hourglass; =readcon-db= owns LMDB multi-reader
screening on CON blobs. Both decode with the same core so semantics never fork.

* How does the hourglass change the ecosystem argument?

Without a linkable multi-language ABI, every engine and optimizer reinvents
text I/O (XYZ, proprietary logs, partial =.con= readers). With the hourglass,
the *same* versioned CON contract embeds into drivers and optimizers (including
codes that historically emitted baroque logs) without choosing Python-only or
Fortran-only interchange. That is the SOTA claim: CON + hourglass is the
correct *default interchange* for optimizer-centric science—not that CON
replaces every internal continuous-MD binary buffer.

* Is CON better than XYZ or ASE I/O for that science?

*Yes.* For optimizer-centric interchange—constraints, sections, multi-language
embedding, CON-backed campaigns—versioned CON via =readcon-core= / =readcon-db=
is strictly preferable to bare XYZ and to ASE =.db= / =ase.io= as the
authoritative path. ASE remains the right calculator hub; XYZ remains a minimal
edge format. Continuous-MD internal binaries (XTC/TRR/DCD) are a different job.
