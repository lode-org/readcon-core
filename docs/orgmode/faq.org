...
=ConFrame= (topology becomes =metadata["bonds"]= when present) and evaluate
chemfiles selection strings on frames via C/C++/Python/Rust APIs. Selection
works for name/type filters without bonds; bond/angle contexts need =bonds=
or an import path that carried topology.
