...
Language APIs: =bindings.org=. On-disk =bonds=: =spec.org=. Projection and
limits: =chemfiles-explain.org=.
...
detail is in =chemfiles-explain.org= (*Why is selection not full chemfiles?*).
