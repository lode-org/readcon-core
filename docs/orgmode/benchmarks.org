...
Ordering against ASE =ase.io.eon= and an eOn-style C reader on the *same CON
text*. Latest measured snapshot (=rgam5terra=, 2026-07-08 UTC, readcon 0.14.0,
...
geometry. Proves the hot path is not a CON-only parlor trick: *ASE plain XYZ is
slower than readcon CON* on the same host.
...
H5MD *does* have Python paths: write/read via **MDAnalysis** (=format="H5MD"=)
and raw datasets via **h5py**. ASE on this host still has no registered =h5md=
...
| h5py =position/value= only | 0.28 | 0.08× | *coords array only* — not cell/constraints/=atom_id=/JSON |
...
#+CAPTION: Feature coverage vs parse speed (measured points; see JSON)
...
#+CAPTION: Feature coverage vs parse speed (measured =readcon= / ASE CON / ASE extXYZ from multiformat_traj; C sscanf from =compare_readers=; ASE CON is not plotted as extXYZ)
[[file:img/pareto_features_vs_speed.svg]]
