...
**Fast cut (what actually matters)**
...
1. *Full multi-frame load:* =read_all_frames= — mmap large files; **Rayon
   ...
   That is the primary throughput path.
2. *Skip when you only need structure:* =count_frames= / =forward_fast= use
   =memchr= newline walking (no atom materialize).
3. *Decimals:* stack buffer + =fast_float2= on atom lines (not hand-rolled SIMD).
4. *Python:* =Python::detach= around Rust parse so Rayon is not GIL-blocked;
   stream with =iter_con= when you process one full frame at a time.
5. *Coordinates after load:* =ConFrame= SoA / Python =coords_array()= on a full
   frame—not a second load API.

Criterion (=cargo bench --bench iterator_bench=) and Cachegrind remain optional
developer tools, not product claims.
