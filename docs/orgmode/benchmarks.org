...
flat SoA fill alone). Remaining cycles are dominated by =fast_float2= and the
AoS =AtomDatum= materialization/drop—not a missing hand-SIMD digit kernel.
Further design headroom: type-index symbols (fewer =Arc= bumps), defer AoS until
writer/Python needs it, or a positions-only multi-frame API that never builds
=atom_data=.
