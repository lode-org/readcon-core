...
** Lean multi-frame positions API (no AoS)

Shipped structural escape hatch when callers only need coordinates (artifact
=benches/results/next_design_terra.json=):

- Rust: =iterators::read_all_positions= / =read_all_positions_str= and
  =parser::parse_single_frame_positions= — owned =(N, 3)= f64 matrices, same
  float kernel as full-frame parse, optional sections structure-skipped, no
  =AtomDatum= / per-atom symbol =Arc=.
- Parallel boundary walk reuses =forward_fast= (same gate as full-frame).
- Python =read_all_positions= now calls the lean Rust path (not
  =read_all_frames= + extract).
- Full-fidelity =read_all_frames= / =ConFrameIterator= unchanged.

Same-session pure-Rust A/B (rgam5terra, release+parallel, median 31):

| Case | Full-frame collect | Lean positions | Speedup |
|------|--------------------|----------------|---------|
| cuh2 ×30 | 0.58 ms | 0.18 ms | ~3.3× |
| cuh2 ×50 | 1.00 ms | 0.28 ms | ~3.6× |
| cuh2 ×100 | 2.16 ms | 0.42 ms | ~5.1× |
| tiny ×200 | 0.18 ms | 0.14 ms | ~1.3× |

Geomean lean vs full **~3.0×**. In-repo tests assert lean positions bit-agree
with full-frame =positions= on fixtures (including vel/forces files for section
skip).
