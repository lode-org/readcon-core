...
- =perf record -g -- … profile_train once= — sample-based cost centers (noisy;
  use to pick *what* to inspect, not to quote a speedup).
- =benches/run_pgo_ab.sh= / =examples/profile_train= — LLVM PGO train/use for a
  *tuned binary*; default =cargo build --release= does not apply PGO.
