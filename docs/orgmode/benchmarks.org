...
Python binding efficiency (terra, robust median-of-medians protocol in
=benches/efficiency_apis_profile.py= / robust runner; artifact
=benches/results/efficiency_improve_terra.json=):

- =count_frames= remains ~6–9% of full =read_all_frames= wall time on the ladder.
- =read_all_positions= uses a bulk f64 SoA copy (no per-row conversion when
  storage is f64) and avoids building Python =Atom= lists; on large ladders it
  is a clear fraction of full batch cost (e.g. ~0.26–0.75× batch in the robust
  run). Prefer it for coordinate-only work; do not treat ratio-to-batch games
  on micro fixtures as absolute speed claims (absolute times are noisy).
- Rust multi-frame parse under Python releases the GIL (=Python::detach=) so
  Rayon is not blocked by the interpreter; this is for concurrent correctness /
  multi-core use, not a guaranteed single-thread wall-clock win.
- Streaming =iter_con= is for incremental processing / lower intermediate
  structure materialization discipline; materializing every frame via the stream
  is not advertised as faster than parallel batch.
- =coords_array= always returns a *fresh* ndarray from the current atom list
  (safe under in-place mutation of prior returns and live atom edits).
