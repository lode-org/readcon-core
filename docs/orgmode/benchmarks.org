...
- Full multi-frame load is always full frames: ~iterators::read_all_frames~
  (mmap above 64 KiB; Rayon multi-frame parse when file size ≥ 48 KiB under the
  ~parallel~ feature). There is no separate “coords-only” public load path.
- Prefer ~count_frames~ / ~forward~ / ~forward_fast~ when the atom payload is not
  needed at all; ~I_refs~ for skip is substantially lower than full parse
  (Cachegrind table).
- Python: ~read_all_frames~ / ~iter_con~ / ~read_first_frame~ for ~ConFrame~
  objects; coordinates via ~ConFrame.coords_array()~ on a loaded frame.

Terra release profile (host rgam5terra; historical artifacts under
=benches/results/efficiency_*=): =count_frames= is a small fraction of
=read_all_frames= wall time. Streaming =iter_con= that materializes every frame
is not faster than batch on parallel-sized files, but avoids holding the full
=PyConFrame= list at once. Criterion: fast-float2 coordinate microbench ~2× std
parse (see =cargo bench --bench iterator_bench=).
...
- *FAST (shipped):* =Python::detach= during Rust multi-frame I/O/parse so Rayon
  and other Python threads are not blocked during =read_all_frames=.
- Concurrent pure-Python worker throughput during long multi-frame loads was
  about 1.44× higher with detach (worker ticks/s) in that harness.
- Streaming =iter_con= remains for incremental use. =coords_array= always returns
  a fresh ndarray from live atoms (no shared cache).
...
flat SoA fill alone). The public model remains **full frames only**
(=read_all_frames= / =ConFrameIterator=); internal assembly shortcuts
(=con_frame_coords_only=, flat f64 fill) do not expose a reduced product.
