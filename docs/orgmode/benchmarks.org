...
|----------------------------+-----------+----------------|
| ASE (=ase.io.eon=) | 35.8 | 1.0x |
| readcon-core (file path) | 4.3 | 8.3x |
| readcon-core (from string) | 4.0 | 9.0x |

#+CAPTION: Parsing throughput: 100 frames x 218 atoms
[[file:img/parsing_throughput.svg]]

The speedup comes from:
- Rust-native parsing (no interpreter overhead)
- fast-float2 for float conversion
- Zero-copy line iteration (borrows from input &str)
- Minimal allocations (Rc<String> per atom type, not per atom)
