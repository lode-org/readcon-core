...
| Julia | from this repo: =julia --project=julia/ReadCon -e 'using Pkg; Pkg.instantiate()'= | [[file:bindings.org][bindings]]; unpack =readcon-core-clib-$VERSION-$target.tar.gz= and set =READCON_CORE_PREFIX= |
| C / C++ / Fortran | CMake FetchContent, Meson wrap, =pkg-config readcon-core=, or the clib tarball | [[file:bindings.org][bindings]] |
