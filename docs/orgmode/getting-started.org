...
| Prebuilt C lib (lean) | =readcon-core-clib-$VERSION-$target.tar.gz= on the GitHub Release | [[file:bindings.org][bindings]] (Windows row is lean; chemfiles not shipped) |
