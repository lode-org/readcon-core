...
=rkr_dlpack_delete=) are the portable path. **Choose output precision and
device** with =RKRDlpackExportOptions= on the =*_dlpack_ex= entry points
(=float_bits= = 32 or 64; =device_type= / =device_id= — only CPU =1= /
=0= is implemented today; other devices return validation error until
device-resident exports land). Legacy =*_dlpack= without options keeps
float64 on CPU. =rkr_frame_copy_*= still fills a caller =double*= buffer
when you already hold f64 storage.
