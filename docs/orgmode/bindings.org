...
*New contributor walk-throughs* (converter XYZ→CON, bond/angle selection
examples in Rust/Python/C, enable flags): see ~docs/orgmode/tutorials.org~
sections *Chemfiles multi-format converter* and *Bond and angle selection*.
Release cutting (cargo-dist PR + crates.io secret): ~docs/orgmode/contributing.org~
*Release process*.
