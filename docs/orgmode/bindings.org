...
with =RKRDlpackExportOptions= on =*_dlpack_ex=: fields are DLPack's own
=DLDataType= (=code= / =bits= / =lanes=, e.g. =kDLFloat= + 32 or 64) and
=DLDevice= (=device_type= / =device_id=, e.g. =kDLCPU=). Only CPU + float32/64
(lanes=1) are implemented for section exports today; other combinations return
=RKR_STATUS_VALIDATION_ERROR=. Legacy =*_dlpack= without options is
=kDLFloat= / 64 on CPU. =rkr_frame_copy_*= still fills a caller =double*=
buffer when you already hold host f64 storage.
