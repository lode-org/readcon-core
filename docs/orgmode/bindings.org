...
**Documented gaps (not claimed parity).** Separate chemfiles display ~name~
vs atomic ~type~ when CON carries one symbol string (~name H1~ may miss after
import; ~type H~ works). Residue/~resname~, extra atom/residue properties,
improper cases needing extra topology CON does not store, and numeric
geometry assertion blocks from chemfiles ~tests/selection.cpp~ are out of
scope unless they pass via projection without CON model changes.
