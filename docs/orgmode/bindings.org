...
| Optional frame ~bonds~ topology | yes | ~PyConFrame.bonds~ / ~has_bonds~ | via metadata | ~rkr_frame_bond_*~ | ~ConFrame::bonds()~ |
| Chemfiles import / selection | yes (~chemfiles~ feature) | ~select_on_frame~ / ~select_atom_indices~ | no | ~rkr_frame_select~ | ~ConFrame::select~ |

Optional chemfiles support (~--features chemfiles~): multi-format trajectory
import into ~ConFrame~, and chemfiles selection grammar evaluation. When
~metadata["bonds"]~ is present, projection sets chemfiles topology so
~bonds:~ / ~angles:~ / ~is_bonded~ selectors work; pair/name selections work
without bonds. Probe support with ~rkr_has_chemfiles_support()~ (C/C++) or
feature flags at build time (Rust/Python).
