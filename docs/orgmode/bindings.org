...
Set =READCON_CORE_LIB= (or the =READCON_LIB_PATH= alias) to the shared
library path, install the =readcon_core= artifact from
=julia/ReadCon/Artifacts.toml.in=, or build with =cargo build --release=
and the Julia package will find it automatically.
