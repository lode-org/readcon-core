...
until implemented. Non-CPU =device_type= (CUDA, …) → =RKR_STATUS_FEATURE_DISABLED=
(struct accepts the full DLPack device space; no GPU allocator yet). Legacy
=*_dlpack= is =kDLFloat=/64 on CPU. =rkr_frame_copy_*= still fills =double*=.
