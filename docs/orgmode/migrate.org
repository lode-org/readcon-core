...
host. Details: [[file:benchmarks.org][benchmarks.org]].
...
ingress is slower than ASE =.traj= (honest) but *faster than ASE NetCDF and
multi-frame XYZ*, and once converted, CON is the fast path for selection,
