...
| C ABI tarball | =c_lib_tarball.yml= | GitHub Release / dispatch | Prebuilt =libreadcon_core= prefix (cargo-c; not cargo-dist) |
...
#+begin_example
  ...
  merge PR to main  ──►  git tag -s vX.Y.Z && git push origin vX.Y.Z
        ...
        └─► workflow "C ABI prefix tarball": cargo-c prefixes (not cargo-dist)
#+end_example
...
| Tag publish | C ABI prefixes | =.github/workflows/c_lib_tarball.yml= (=scripts/package-clib.sh=; CLI stays cargo-dist) |
