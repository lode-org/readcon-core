...
| C lib tarball | =c_lib_tarball.yml= | =v*= tag, PR, dispatch | cargo-c prefixes (manylinux/macOS/Windows; Windows chemfiles explicit) |
...
2. The =asv-combine= job runs
   [[https://github.com/airspeed-velocity/asv_spyglass][asv-spyglass]] =compare= on the two result JSONs →
...
3. =ci_bench_commenter.yml= triggers on =workflow_run= completion when the
   ...
   posts the table with [[https://github.com/HaoZeke/asv-perch][asv-perch]].
...
[[file:benchmarks.org][benchmarks.org]] (=benchmarks/=, =asv.conf.json=).
...
#+begin_example
  ...
  merge PR to main  ──►  git tag -s vX.Y.Z && git push origin vX.Y.Z
        ...
        ├─► workflow "C library tarball": cargo-c prefixes (does not publish wheels)
...
| Tag publish | Prebuilt C library prefixes | =.github/workflows/c_lib_tarball.yml= (separate from PyPI wheels) |
...
- *Secret*: repository secret *exactly named* =CARGO_REGISTRY_TOKEN=. Value is
  ...
  (create at https://crates.io/settings/tokens ). Without it, the publish
...
9. Tag CI also runs =python_wheels.yml= (PyPI), =c_lib_tarball.yml=
   (prebuilt cargo-c prefixes; does *not* publish wheels), and
   cargo-dist =Release= (CLI tarballs + GitHub Release body from the
   changelog).
...
10. Verify all distribution channels:
    ...
    - [[https://github.com/lode-org/readcon-core/releases][GitHub Releases]] has cargo-dist archives
      plus =readcon-core-clib-*= prefixes and cxx source tarballs

Optional local C ABI tarball for consumers that do not wait for tag CI
(headers + =libreadcon_core.{a,so}= via =cargo cinstall= /
=scripts/package-clib.sh=) can still be attached with =gh release upload=
if needed. Do not fold that job into =python_wheels.yml=.
