...
3. =ci_bench_commenter.yml= triggers on =workflow_run= completion, downloads
   =benchmark-results=, and posts the table with [[https://github.com/HaoZeke/asv-perch][asv-perch]].
...
local reproduce: [[file:benchmarks.org][benchmarks.org]] (=benchmarks/=, =asv.conf.json=).
