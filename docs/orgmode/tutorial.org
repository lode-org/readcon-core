...
#+description: One Good Tutorial: install, read a multi-frame CON, inspect, write a round-trip, build a frame with energy.
...
can repeat in any language (see [[file:howto.org][howto]]).
...
| XYZ / PDB / GRO → CON | [[file:chemfiles-tutorial.org][chemfiles-tutorial]] |
| Declared sections (=forces=, =charges=, …) | [[file:faq.org][faq]], [[file:spec.org][spec]] |
| Full API tables | [[file:bindings.org][bindings]] |
| Why CON looks this way | [[file:evolution.org][evolution]], [[file:architecture.org][architecture]] |
