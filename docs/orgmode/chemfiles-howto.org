...
**Python:** prefer producing CON via Rust/CLI once, or install
=readcon-chemfiles= and use selection on an existing CON. Path import is not
wrapped as a single Python function in 0.13; use the Rust API or a small
binary for batch ingress.
