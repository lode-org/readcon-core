#+title: Tutorial — Convert other formats into CON (chemfiles)
#+options: toc:2 num:t
#+description: Learning-oriented walkthrough: install full Python or Rust chemfiles support, convert XYZ (and friends) to CON, preserve bonds, run selection, write a trajectory.

/Diátaxis: *tutorial* (learning). Goal: succeed once end-to-end. For goals like
"only convert PDB" see [[file:chemfiles-howto.org][chemfiles how-to guides]]. For *why* lean vs full wheels and
index remap, see [[file:chemfiles-explain.org][chemfiles explanation]]. For API tables see
[[file:chemfiles-reference.org][chemfiles reference]] and [[file:bindings.org][bindings]]./
...
** Path C — Editable Python from source
...
* Step 2 — Convert XYZ → CON frame (Rust)
...
* Step 3 — Same conversion mindset in Python

The published =readcon-chemfiles= wheel links libchemfiles, but **path import
helpers are exposed primarily on the Rust/C side today**. In Python for this
tutorial we:

1. Confirm chemfiles support.
2. Prefer writing CON from Rust once, *or* build a frame with bonds and
   demonstrate **selection** (the selection APIs are fully wired in Python).
3. Show reading the CON you produced and selecting on it.

#+begin_src python
import readcon

assert readcon.has_chemfiles_support(), "install readcon-chemfiles, not lean readcon"

# Read the CON produced in Step 2 (or any CON with optional bonds)
frame = readcon.read_first_frame("water_from_xyz.con")
print("atoms", len(frame.atoms), "has_bonds", frame.has_bonds)
if frame.has_bonds:
    print("n_bonds", len(frame.bonds))

# Selection uses chemfiles grammar on the projected frame (needs bonds for angles)
oxy = readcon.select_atom_indices(frame, "name O")
print("oxygen atom_data indices", oxy)

# If bonds are present (from a format that carried topology, or hand-authored CON)
try:
    angles = readcon.select_on_frame(frame, "angles: all")
    print("angles", len(angles["matches"]), "arity", angles["context_size"])
    for m in angles["matches"][:3]:
        print("  angle atoms", m)
except RuntimeError as e:
    print("angles selection:", e)
    print("(XYZ often has no bonds — see Step 4 to add topology or use PDB)")
#+end_src

Plain XYZ usually has **no bonds**. That is normal: conversion still gives you
CON geometry; topology-aware selectors need bonds (next step or a richer
format).

* Step 4 — Drive topology: prefer formats with bonds, or set bonds in CON
...
**4b — Or attach bonds on a builder after ingress (indices = atom_data order)**
...
**4c — Python selection on that CON**
...
r = readcon.select_on_frame(frame, "bonds: all")
print("bonds matches", r["matches"])
r = readcon.select_on_frame(frame, "angles: all")
print("angles matches", r["matches"])
#+end_src

* Step 5 — Multi-format conversion habit (other ingress formats)
...
Next: [[file:chemfiles-howto.org][how-to guides]] (specific formats, C-only, lean wheel limits) and
[[file:chemfiles-explain.org][explanation]] (why two PyPI names, index remap, metadata bonds).
