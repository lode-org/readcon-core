...
/Executable twin: =docs/notebooks/chemfiles_ingress.py= (Papermill) and
=docs/orgmode/chemfiles-notebook.org= (Org Babel). Run =scripts/run-chemfiles-notebook.sh=.
...
* Step 3 — Convert XYZ → CON entirely in Python
