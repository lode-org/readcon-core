...
/Diátaxis: *reference* (information-oriented). For learning see
...
| =select_on_frame(frame, sel)= | =RuntimeError= | dict |
| =select_atom_indices(frame, sel)= | =RuntimeError= | =list[int]= |
| =PyConFrame.bonds= / =has_bonds= | yes | yes |

=select_on_frame= dict keys: =selection=, =context_size=, =matches=
(=list[list[int]]=), =primary_indices=.
