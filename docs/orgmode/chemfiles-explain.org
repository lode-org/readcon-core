...
| Out of scope | Why |
|--------------|-----|
| Residue / =resname= (and most residue-centric selectors) | CON has no residue table; import does not synthesize one for selection |
| Full chemfiles per-atom / per-residue property surface | Import stores only a subset under =chemfiles_atom_properties= / frame metadata |
| Improper torsions and other topology beyond undirected =bonds= pairs | On-disk CON stores pair bonds only |
| Numeric geometry *assertion* suites from upstream =tests/selection.cpp= (=distance= / =angle= / =dihedral= / =out_of_plane= thresholds with extra atoms) | We do not claim line-by-line parity with every chemfiles unit test; trivial projections may work by accident |

For the supported API matrix and language surfaces, see [[file:bindings.org][Language bindings]]
and [[file:chemfiles-reference.org][Reference — Chemfiles conversion and selection]].
