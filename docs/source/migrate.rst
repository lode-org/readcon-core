...
Measured: faster than hand-rolled / ASE ASCII — including plain XYZ
-------------------------------------------------------------------

Equal geometry (same atoms, same frame count). ASE reads XYZ/extXYZ/CON;
readcon reads CON. This is not a “CON vs CON only” trick.
...
Snapshot on ``rgam5terra`` (2026-07-08), readcon 0.14.0, ASE 3.29.0, median of 5,
100 frames × 218 atoms (geometry from ``cuh2.con``):
...
.. table::
    ...
    +--------------------------+-----------+--------------+
    | ASE CON (``ase.io.eon``) |      30.5 | 12.0× slower |
    +--------------------------+-----------+--------------+

Same host, CON-peer harness (``benches/compare_readers.py``, 100×218 generated
trajectory): ASE CON 30.6 ms, C sscanf 7.3 ms, readcon file path **3.3 ms**
(9.2× ASE, 2.2× C). CI Cachegrind tracks I-refs
