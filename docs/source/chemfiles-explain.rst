...
CON is a complete checkpoint format for eOn / LODE (typed blocks, fixed
flags, velocities / forces / energies, JSON line-2 metadata). Structures
still often arrive from elsewhere as XYZ, PDB, GRO, LAMMPS dumps, and other
chemfiles-readable layouts.
...
**CON fidelity** (atom\_id, sections, writer, FFI, NEB-friendly builders). The
job is to **land foreign trajectories in CON** for eOn, amsel, and CON-native
tools.
