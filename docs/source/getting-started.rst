...
``readcon-core-cxx-$VERSION.tar.gz`` (FetchContent / wrap). The prebuilt
C ABI prefix is ``readcon-core-clib-$VERSION-$TARGET.tar.gz``
(``c_lib_tarball.yml``, not cargo-dist) for Julia, Fortran/fpm, and
pkg-config.
