...
is the one-command install for C / Fortran / Julia. cargo-dist ships
CLI archives only; it does not emit this prefix. Windows clib
tarballs are lean: chemfiles is not included
(``RKR_STATUS_FEATURE_DISABLED``). Maintainers only:
