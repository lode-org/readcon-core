...
prefix (``scripts/package-clib.sh`` → ``readcon-core-clib-$VERSION-$TARGET.tar.gz``
via ``c_lib_tarball.yml``) is the Julia / Fortran / pkg-config URL.
cargo-dist 0.28 ships CLI archives only and is not forced to produce
cargo-c artifacts. Maintainers only: ``scripts/regen-capi-headers.sh``.
