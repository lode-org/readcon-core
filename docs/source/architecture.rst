...
GitHub Release) is the FetchContent / wrapdb URL. The lean cargo-c
prefix tarball (``scripts/package-clib.sh`` →
``readcon-core-clib-$VERSION-$target.tar.gz``) is the prebuilt shared
library for Julia/Fortran/``pkg-config``. The Windows row of that
matrix is the lean DLL; chemfiles is not shipped for Windows there.
Maintainers only: ``scripts/regen-capi-headers.sh``.
