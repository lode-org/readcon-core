...
tarball (``scripts/package-clib.sh`` → ``readcon-core-clib-$VERSION-$target.tar.gz``)
is the one-command Julia / Fortran / pkg-config path;
``scripts/check-clib-dist.sh`` unpacks it and asserts headers, ``libreadcon_core``,
and ``readcon-core.pc`` without cbindgen. Maintainers only:
