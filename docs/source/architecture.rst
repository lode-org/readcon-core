...
GitHub Release) is the FetchContent / wrapdb URL. The prebuilt C
prefix (``scripts/package-clib.sh`` →
``readcon-core-clib-$VERSION-$target.tar.gz``) is attached by
``c_lib_tarball.yml``. cargo-dist 0.28 CLI archives do not carry that
prefix: ``package-libraries`` only places the cdylib next to the CLI
binary, with no headers or pkg-config. Maintainers only:
