...
Why CON v2 and v3 look the way they do. Each section names the
rejected alternatives, then the constraint that decided the layout.
Independent implementers should be able to reconstruct the same
choices from this page and the spec.
...
- New section types require no format-level changes: add a string
  to the array.
...
The optional ``validate=true`` metadata flag lets producers ask readers
to verify that declared sections match coordinate blocks exactly:
component symbols, labels, fixed masks, atom ids, numeric finiteness,
metadata schema, and physical header invariants must all agree. This
...
Most potentials (EMT, EAM, DFT) produce a total energy, not a local
decomposition. The per-frame ``energy`` metadata key covers that case.
When a potential does supply per-atom contributions, writers emit the
optional ``energies`` section; the two coexist.

The ``potential`` metadata key records provenance
(``{"type":"EMT","params":{"cutoff":6.0}}``) so energy and force values
stay interpretable without a sidecar README.
...
zstd support is available behind the optional ``zstd`` Cargo feature
since v0.10.0, using the same magic-byte detection pattern (``28 b5 2f fd`` for zstd frames; ``1f 8b`` for gzip). ``.con.zst`` files are read and
written through ``ConFrameWriter::from_path_zstd`` and the same
``read_all_frames`` entry point. Builds without the feature still detect
...
Why text CON stays the on-disk authority (not bcon, not RCSO)
-------------------------------------------------------------

Alternatives considered
~~~~~~~~~~~~~~~~~~~~~~~

1. **bcon (Cap'n Proto on disk)**: serialize ``ConFrame`` with the existing
   ``schema/ReadCon.capnp`` layout (or a packed sibling) so a campaign
...
3. **readcon-db RCSO**: cooked SoA numerics inside LMDB. Fast screening
   (energy, formula, section presence). Not a round-trippable
   checkpoint: constraints, ``atom_id``, JSON keys, and generator
   comments are not the store's job.

Decision
~~~~~~~~

Text ``.con`` / ``.convel`` (optionally gzip or zstd) remains the
authoritative interchange. Three reasons:

- Independent implementers already read the 9-line header and the
  5-column atom line. A binary schema would split the ecosystem into
...
- Parse column 4 as a boolean: any nonzero value is "fixed". Values
  0 and 7 match free and all-fixed. Values 1-6 (partial masks on
  spec 2) over-constrain y/z on a v1 reader.
...
1. **Bump the entire grammar** (new extensions, binary framing): rejected.
   It breaks human inspectability and the eOn/LODE interoperability
   that motivated CON.

2. **Leave units forever optional**: rejected for **new** writes.
   Downstream optimizers and ML tools mis-scale energies and forces
   silently.

3. **Encode units only in filenames or READMEs**: rejected. Campaign
   stores lose the pairing.
...
Optional sections beyond velocities / forces / energies
-------------------------------------------------------
...
Conformance corpus keyed to spec clauses
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The metadata schema (``schema/con-metadata.schema.json``) machine-checks
line 2. Whole-file conformance still rests on the reference test suite.
A published corpus of minimal valid and invalid files, each annotated with
the normative clause it exercises and the required outcome
(parse / specific ``ParseError`` variant), lets an independent implementation
claim conformance without reading readcon-core source.

Vocabulary registry over ad hoc keys
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Reserved metadata keys live in one table in the spec. A registry
(one file, one key per row: name, type, since-version, producer semantics)
with an extension-key convention (``x_`` prefix) keeps third-party producers
from colliding on bare names while unrecognized-key preservation stays
mandatory.
