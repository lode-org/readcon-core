...
Equal-geometry trajectory (``rgam5terra`` 2026-07-08, 100×218, median of 5):

- ASE multi-frame **plain XYZ** ``read``: **26.8 ms**

- ``readcon.read_chemfiles`` (XYZ → CON frames): **13.3 ms** (**2.0×** ASE XYZ)

- ``readcon.read_con`` (native CON): **2.5 ms** (**10.6×** ASE XYZ)

Also: ASE XYZ/extXYZ/CON full parse still ~10–12× slower than native CON
(``multiformat_traj.py``); CON peers (``compare_readers.py``): readcon 3.3 ms vs ASE
CON 30.6 ms (9.2×) and C sscanf 7.3 ms (2.2×). Recipes:
`benchmarks <benchmarks.rst>`_ · `migrate <migrate.rst>`_ ·
``benches/ase_xyz_vs_chemfiles_con.py``.
