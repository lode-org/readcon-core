...
Language APIs: ``bindings``. On-disk ``bonds``: ``spec``. Projection and
limits: ``chemfiles-explain``.
...
languages share one evaluator. Topology-oriented cases from chemfiles
``tests/selection.cpp`` land in ``chemfiles_selection_cpp_regression`` as
*multisets* of ``atom_data`` indices (after ``atom_id`` remap), not as identical
chemfiles index lists.

On *import*, display names such as ``H1`` versus types such as ``H`` are kept in
``chemfiles_atom_names`` / ``chemfiles_atom_types``; the on-disk column is still a
single ``symbol``. Hand-built frames without those keys use ``symbol`` for both.

The ``con`` format has no residue table, stores only pair ``bonds``, and keeps a
thin property subset on import. Consequently ``resname``, most chemfiles
properties, impropers, and the geometry assertion blocks in
``tests/selection.cpp`` are not part of the CON selection surface. Mechanistic
detail is in ``chemfiles-explain`` (*Why is selection not full chemfiles?*).
