...
// allocation alive via Arc (dlpk `ArcArray -> DLPackTensor` shares data; not a deep memcpy).
// Mutating the builder may CoW-fork builder storage without rewriting an exported tensor.
// lifetime), not an owned `DLManagedTensorVersioned*` for FFI.
//
