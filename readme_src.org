...
[[https://github.com/lode-org/readcon-db][readcon-db]] (=cargo add readcon-db=, =pip install readcon-db=,
https://lode-org.github.io/readcon-db/); blobs remain CON text.

| Layer | Crate | Responsibility |
|-------+-------+----------------|
| Format I/O | =readcon-core= / Python =readcon= | Spec v2–v3 parse/write, hourglass FFI, optional chemfiles import |
| Corpus | [[https://github.com/lode-org/readcon-db][readcon-db]] / =readcon_db= | Heed/LMDB store, secondary indexes, exact-match dedup, multi-language CLI |

Rust rewrite of [[https://github.com/HaoZeke/readCon][readCon]]. Optional chemfiles import maps foreign structure
...
[[file:docs/orgmode/benchmarks.org][docs/orgmode/benchmarks.org]] and [[file:docs/orgmode/faq.org][docs/orgmode/faq.org]] (*Objective*).
...
| Area | readcon-core |
|------+--------------|
| Payload | Velocities, forces, per-direction constraints, =atom_id=, versioned JSON metadata on every binding |
| Languages | One =rkr_*= C surface for Fortran / C / C++ / Python / Julia |
| Spec | v2–v3, =validate=true=, declared sections, typed metadata helpers |
| Measurements | CI Cachegrind I-refs; CON peer timing vs ASE =ase.io.eon= and eOn-style C sscanf |
| Campaigns | [[https://github.com/lode-org/readcon-db][readcon-db]] with CON text as durable identity |
| Import | Optional chemfiles → CON |

Predecessor: [[https://github.com/HaoZeke/readCon][readCon]]. ASE workflows may use =ase.io.eon= or the optional
