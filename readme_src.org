...
v2-v3, [[file:docs/orgmode/spec.org][docs/orgmode/spec.org]]).
...
- *Bindings:* Python (PyO3), Julia (ccall), C (shipped header), C++ (RAII header), Fortran (fpm); hourglass ABI patterned on [[https://github.com/metatensor/metatensor][metatensor]].
...
| C ABI tarball | =readcon-core-clib-$VERSION-$target.tar.gz= on the GitHub Release | same; no cbindgen; Julia/Fortran set =READCON_CORE_PREFIX= / =pkg-config= |
...
- *Hourglass FFI:* shipped C header plus a hand-written C++ RAII wrapper, same pattern as [[https://github.com/metatensor/metatensor][metatensor]]. CMake FetchContent, Meson wrap, and =readcon-core.pc= do not run cbindgen.
...
** Citation
If you use ~readcon-core~ in academic work, please cite it via the metadata in [[file:CITATION.cff][CITATION.cff]]. A Zenodo DOI is minted on a freeze tag and recorded in =CITATION.cff= identifiers; this tree does not invent one.
