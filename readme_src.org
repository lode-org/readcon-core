...
| =rkr_*= C ABI | C / C++ / Python / Julia / Fortran: one parser, one wire |
...
- *Bindings:* Python (PyO3), Julia (ccall), C (committed header), C++ (RAII header), Fortran (fpm). Same C ABI shape as [[https://github.com/metatensor/metatensor][metatensor]].
...
- *C ABI:* C header plus a hand-written C++ RAII wrapper, same pattern as [[https://github.com/metatensor/metatensor][metatensor]]. CMake FetchContent, Meson wrap, and =readcon-core.pc= do not run cbindgen.
