...
grep -q 'check-clib-dist.sh' .github/workflows/c_lib_tarball.yml \
    || die "c_lib_tarball.yml must run scripts/check-clib-dist.sh on the archive"

# Windows chemfiles stays the official prebuilt, never from-sources.
...
grep -q 'windows-2022' .github/workflows/c_lib_tarball.yml \
    || die "c_lib_tarball.yml must list Windows chemfiles explicitly"
grep -q 'advapi32' .github/workflows/c_lib_tarball.yml \
    || die "c_lib_tarball.yml must link advapi32 for Windows chemfiles"
